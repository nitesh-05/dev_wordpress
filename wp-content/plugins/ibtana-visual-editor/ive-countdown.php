...
class WP_Ivecountdown {
	...
	function ive_shortcode($atts, $content=null) {
	  ...
	  // deal with display years
		if(!empty($interval->y) && $displayyears != 'false'){
			// if no months, calculate everyting with total days
			if($displaymonths != 'false'){
		...
		if(!empty($date_arr['months']) && $displaymonths != 'false'){
			if(!empty($interval->y) && $displayyears == 'false'){
		...
		else if(!empty($next_arr['next_month']) && $displaymonths != 'false'){
			$next_arr['next_day'] = $tomorrow_interval->days;
		}

		// adjust days if weeks are being displayed
		if($displayweeks == 'false'){
		...
		if(!empty($date_arr['years']) && $displayyears == 'false'){
