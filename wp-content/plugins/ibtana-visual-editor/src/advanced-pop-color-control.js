 ...
 * Import Icons
 */
import cIcons from './ibtana-color-icons';
import IbtanaColorPicker from './ibtana-color-picker';
...
class AdvancedPopColorControl extends Component {
	...
	render() {
		...
		return (
			<div className="ive-color-popover-container components-base-control new-ibtana-advanced-colors">
				<div className="ive-advanced-color-settings-container">
					{ this.props.label && (
						<h2 className="ive-beside-color-label">{ this.props.label }</h2>
					) }
					...
					<div className="ive-beside-color-click">
						{ this.state.isVisible && (
							<Popover position="top left" className="ive-popover-color new-ibtana-advanced-colors-pop" onClose={ toggleClose }>
						...
						{ this.state.isVisible && (
							<Tooltip text={ __( 'Select Color' ) }>
								<Button className={ `ive-color-icon-indicate ${ ( this.state.alpha ? 'ive-has-alpha' : 'ive-no-alpha' ) }` } onClick={ toggleClose }>
									<ColorIndicator className="ive-advanced-color-indicate" colorValue={ currentColorString } />
						...
						{ ! this.state.isVisible && (
							<Tooltip text={ __( 'Select Color' ) }>
								<Button className={ `ive-color-icon-indicate ${ ( this.state.alpha ? 'ive-has-alpha' : 'ive-no-alpha' ) }` } onClick={ toggleVisible }>
									<ColorIndicator className="ive-advanced-color-indicate" colorValue={ currentColorString } />
									{ '' === currentColorString && this.state.inherit && (
										<span className="color-indicator-icon">{ cIcons.inherit }</span>
									) }
