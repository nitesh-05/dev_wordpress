...
const changeBodyClass = ( key ) => {
	document.body.classList.remove( 'ive-editor-width-default' );
	document.body.classList.remove( 'ive-editor-width-nosidebar' );
	document.body.classList.remove( 'ive-editor-width-sidebar' );
	document.body.classList.remove( 'ive-editor-width-fullwidth' );
	document.body.classList.add( 'ive-editor-width-' + key );
};
	...
	return (
		<Fragment>
			<PluginSidebarMoreMenuItem
				target="ive-editor-width"
			>
				Editor Max Width
			</PluginSidebarMoreMenuItem>
			<PluginSidebar
				name="ive-editor-width"
				title="Editor Max Width"
			>
				<div className="ibtana-visual-editor-width-control">
					<h2 className="ibtana-visual-editor-width-heading">{ __( 'Editor Max Width' ) }</h2>
					<ButtonGroup aria-label={ __( 'Editor Max Width' ) }>
						{ map( editorWidthOptions, ( { name, key } ) => (
							<Button
								key={ key }
								className="ive-editor-width-btn"
								isSmall
								isPrimary={ meta.ibtana_visual_editor_editor_width === key || ( '' === meta.ibtana_visual_editor_editor_width && 'default' === key ) }
								aria-pressed={ meta.ibtana_visual_editor_editor_width === key || ( '' === meta.ibtana_visual_editor_editor_width && 'default' === key ) }
								onClick={ () => {
									changeBodyClass( key );
					...
					<p>{ __( '*note: You can define these widths in your Ive Blocks settings.' ) }</p>
				</div>
			</PluginSidebar>
		</Fragment>
	);
	...
	withDispatch( ( dispatch ) => ( {
		onUpdateWidth( value, newMeta, oldMeta ) {
			const meta = {
				...
				ibtana_visual_editor_editor_width: value,
			};
			if ( 'default' === value ) {
				dispatch( 'core/editor' ).updateEditorSettings( { maxWidth: ive_blocks_params.default_size } );
			} else if ( 'sidebar' === value ) {
				dispatch( 'core/editor' ).updateEditorSettings( { maxWidth: ive_blocks_params.sidebar_size } );
			} else if ( 'nosidebar' === value ) {
				dispatch( 'core/editor' ).updateEditorSettings( { maxWidth: ive_blocks_params.nosidebar_size } );
...
registerPlugin( 'ive-editor-width', {
	icon: icon,
	render: plugin,
} );
